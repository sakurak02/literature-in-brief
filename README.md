...
メタ情報は上記9項目を記入してください。number は重複しない3桁の整理番号、date は YYYY-MM-DD 形式、region は日本文学なら japan、海外文学なら foreign を指定します。
...
作品番号や表示順を変更するときは、各作品の `index.md` にある number の値だけを書き換えます。number はslug・フォルダ名・公開URL・画像パスには使われません。
ペン画と「静かな美術館」リンクは自動表示されるため、本文への記入は不要です。
作品一覧は region ごとに「日本文学」「海外文学」へ分かれ、各区分内で number の小さい順に自動生成されます。date と region は作品カードには表示されません。HTMLや一覧データの編集は不要です。
